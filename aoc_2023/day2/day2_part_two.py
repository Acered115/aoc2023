...
def minimum_finder(game):
    ...
    for item in game["sets"]:
        # print(item)
        for colour in minimum_colour.keys():
            if minimum_colour[colour] < item[colour]:
                minimum_colour[colour] = item[colour]

    # print(minimum_colour)

    power = 1
    for x in minimum_colour.values():
        power *= x
    return minimum_colour, power
...
def game_parser(line: str):
    ...
    power = minimum_finder(Game)[1]
    return (Game["game"], possible, power)


if __name__ == "__main__":
    # game_parser("Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green\n")
    sum_of_game_colour_powers = 0
    with open("puzzle_inputs/day2.txt") as file:
        for line in file:
            # print(repr(line))
            game_result = game_parser(line)
            # if game_result[1] == True:
            sum_of_game_colour_powers += game_result[-1]
            print(game_result)
    print(sum_of_game_colour_powers)
